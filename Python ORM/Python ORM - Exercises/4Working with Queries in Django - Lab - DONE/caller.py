...
def order_books_by_year():
    ...
    sorted_books = sorted(books, key=lambda book: (book.publication_year, book.title))
...
def filter_authors_by_nationalities(nationality):
    ...
    sorted_authors = sorted(authors, key=lambda author: (author.first_name, author.last_name))
...
def filter_authors_by_birth_year(year1, year2):
    ...
    for author in all_authors:
        if author.birth_date:
            birth_date = str(author.birth_date).split("-")
            year = int(birth_date[0])
            if year1 <= year <= year2:
                wanted_authors.append(author)
    sorted_authors = sorted(wanted_authors, key=lambda author: (author.birth_date))
    for author in reversed(sorted_authors):
        result.append(f"{author.birth_date}: {author.first_name} {author.last_name}")
    return "\n".join(result)


def change_reviewer_name(current_reviewer, new_reviewer):
    reviews = Review.objects.all()
    for review in reviews:
        if review.reviewer_name == current_reviewer:
            review.reviewer_name = new_reviewer
            review.save()
