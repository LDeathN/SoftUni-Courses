...
class VideoGame(models.Model):
    ...
    def clean(self):
        if self.rating is not None and (self.rating < 0.0 or self.rating > 10.0):
            raise ValidationError("The rating must be between 0.0 and 10.0")

        if self.release_year is not None and (self.release_year < 1990 or self.release_year > 2023):
...
class Exercise(models.Model):
    ...
    def get_exercises_within_duration(cls, min_duration: int, max_duration: int):
        return cls.objects.filter(models.Q(duration_minutes__gte=min_duration) & models.Q(duration_minutes__lte=max_duration))

    @classmethod
    def get_exercises_with_difficulty_and_repetitions(cls, min_difficulty: int, min_repetitions: int):
        return cls.objects.filter(models.Q(difficulty_level__gte=min_difficulty) & models.Q(repetitions__gte=min_repetitions))
