...
for element in range(0, len(numbers)):
    if element % 2 == 0:
        even.append(numbers[element])
    else:
        odd.append(numbers[element])
count = 0

while text != []:
    result = text[0:int(even[count])]
    for i in range(0, int(even[count])):
        text.remove(text[0])
    skip = text[0:int(odd[count])]
    for i in range(0, int(odd[count])):
        text.remove(text[0])
...
for i in attacks:
    row = int(i[0])
    column = int(i[2])
    if int(field[row][column]) >= 1:
        (field[row][column]) = int(field[row][column]) - 1
        if int(field[row][column]) == 0:
            destroyed += 1
print(destroyed)
...
def dfs(board, visited, row, col):
    if row < 0 or row >= len(board) or col < 0 or col >= len(board[0]) or board[row][col] == '-' or visited[row][col]:
        return 0

    visited[row][col] = True
    count = 1

    # Recursively visit adjacent dots
    count += dfs(board, visited, row - 1, col)  # Up
    count += dfs(board, visited, row + 1, col)  # Down
    count += dfs(board, visited, row, col - 1)  # Left
    count += dfs(board, visited, row, col + 1)  # Right
...
for row in range(n):
    for col in range(len(board[0])):
        if board[row][col] == '.' and not visited[row][col]:
            current_count = dfs(board, visited, row, col)
